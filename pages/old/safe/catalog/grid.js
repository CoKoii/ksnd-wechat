...
Page({
  ...
  getList(){
    ...
    app.api.reqRptCatalog(param).then(res=>{
      ...
      for(let item of newList){//网格管理员仅展示管理的网格
        if(this.data.userType=='10001003'){
          if(item.auth == 1){
            dataList.push(item)
          }
        }else{
  ...
  onHandle(e){
    ...
    wx.navigateTo({
      url:'/pages/old/safe/catalog/project?upid='+id+'&auth='+auth
    })
  },
  onCheckN(e){
    ...
    const {id,auth} = e.currentTarget.dataset
    wx.navigateTo({
      url:'/pages/old/safe/handle/danger-list?range=grid&upid='+id+'&auth='+auth
    })
  },
})