...
Page({
  ...
  init(){
    ...
    app.api.reqRptGet(param).then(res=>{
      ...
      console.log('loadfiles',that.data.dataForm)
    }).catch(e=>{
  ...
  previewImage: function (e) {
    ...
    console.log('previewImage:',field,link)
    wx.previewImage({
  ...
  onSubmit(e){
   ...
   //处理附件
    let fileStr = ''
    if(this.data.dataForm.improvePics_files){
      for(let file of this.data.dataForm.improvePics_files){
        fileStr += file.file+','
      }
    }
    param.improvePics=fileStr.replace(/,$/gi,"")
    console.log('submit',param)
    if(!param.improveMemo||param.improveMemo==''){
      wx.showToast({title: '请填写整改说明',icon:'error'})
      return
    }
    if(!param.improvePics||param.improvePics==''){
      wx.showToast({title: '请上传整改图片',icon:'error'})
      return
    }
    app.api.reqRptImprove(param).then(res=>{
      if(res.cnt==1){
